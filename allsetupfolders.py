...
while setupitem < 128:
    ...
    while datablock == []:
        whichcall = "getSetupItems"

        parvalue = str(setupitem)
        # CREATE THE CALL
        url = ('https://api.boldchat.com/aid/' + str(setAuth.aid) + '/data/rest/json/v2/'+ str(whichcall)+'?auth=' + str(setAuth.auth)+'&FolderID='+'&'+str(parvalue) + '&FromDate=2019-10-14T00:00:00.397Z&ToDate=2019-10-14T20:39:32.397Z')
        r = requests.get(url)
        Response = r.json()
        #print(url)
        print(setupitem, Response['Status'])

        parkey =''
        parvalue = str(setupitem)

        #if the response gets an error
        while Response['Status'] == 'error' and setupitem < 128:
            print(setupitem, Response['Status'])
            print()
            ...
            if str.split(Response['Message'])[0] == 'Missing': 
                ...
                url = ('https://api.boldchat.com/aid/' + str(setAuth.aid) + '/data/rest/json/v2/'+ str(whichcall)+'?auth=' + str(setAuth.auth)+'&'+str(parkey) + '='+ str(parvalue)+'&FromDate=2019-10-14T00:00:00.397Z&ToDate=2019-10-14T20:39:32.397Z')
                r = requests.get(url)
                Response = r.json()
                setupitem += 1
                parvalue = str(setupitem)
    ...
    #When truncated, handle paging
        ...
        while truncated == True:
            ...
            url = ('https://api.boldchat.com/aid/' + str(setAuth.aid) + '/data/rest/json/v2/'+str(whichcall)+'?auth=' + str(setAuth.auth)+'&'+str(parkey) + '='+ str(parvalue)+'&NextPage='+str(nextpage)+'&FromDate=2019-10-14T00:00:00.397Z&ToDate=2019-10-14T20:39:32.397Z')
            r = requests.get(url)
            Response = r.json()
            #print(nextpage)
