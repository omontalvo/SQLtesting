...
while datablock == []:
    ...
    dateinfo = '&FromDate=2021-09-01T00:00:00.000Z&ToDate=2021-09-30T00:00:00.000Z'#T00:00:00.000Z'

    # CREATE THE CALL
    url = ('https://api.boldchat.com/aid/' + str(setAuth.aid) + '/data/rest/json/v2/'+ str(whichcall)+'?auth=' + str(setAuth.auth)+dateinfo)
    r = requests.get(url)
    Response = r.json()
    ...
    while Response['Status'] == 'error':
        ...
        if str.split(Response['Message'])[0] == 'Missing': 
            ...
            url = ('https://api.boldchat.com/aid/' + str(setAuth.aid) + '/data/rest/json/v2/'+ str(whichcall)+'?auth=' + str(setAuth.auth)+'&'+str(parkey) + '='+ str(parvalue)+dateinfo)
            r = requests.get(url)
            Response = r.json()
        else:
...
#When truncated, handle paging
    ...
    while truncated == True:
        ...
        url = ('https://api.boldchat.com/aid/' + str(setAuth.aid) + '/data/rest/json/v2/'+str(whichcall)+'?auth=' + str(setAuth.auth)+'&'+str(parkey) + '='+ str(parvalue)+'&NextPage='+str(nextpage)+dateinfo)
        r = requests.get(url)
        Response = r.json()
        #print(nextpage)
