...
def maketablesingle(table_name,calldict):
    c.execute("SELECT name FROM sqlite_master WHERE type='table';")
    tablenames = c.fetchall()
    tablelist = []
    for tables in tablenames:
...
conn = sqlite3.connect('scratch6.db')
c = conn.cursor()
table_name = whichcall + setAuth.aid + parvalue
...
if type(calldict) == list:
    ...
    maketablesingle(table_name,calldict)